...
//	划过图片改变路径
	var lis1 = $("#uls1")[0].getElementsByTagName("li");
	var lis2 = $("#uls2")[0].getElementsByTagName("li");
	console.log(lis1)
	for(let i =0; i<lis2.length; i++){
		lis2[i].onmousemove = function(){
			...
			for(let i=0; i<lis1.length; i++){
				...
				lis2[i].style.border = "1px solid #fff"
			}
			lis1[this.index].style.display = "block";
			lis2[this.index].style.border = "1px solid red"
		}
	}
//购物车.的  添加 与减少
		var num = 0;
	$("#jian")[0].onclick = function(){
		if ($("#zhi")[0].innerHTML > 0) {
			num --;
			$("#zhi")[0].innerHTML = num;	
		}
	}
	$("#add")[0].onclick = function(){
		if ($("#zhi")[0].innerHTML <=3) {
			num ++;
			$("#zhi")[0].innerHTML = num;	
		}
	}
